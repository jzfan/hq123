<html xmlns="http://www.w3.org/1999/xhtml"><head>
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.0; user-scalable=0;">
<title>乾源通首页</title>
<meta name="keywords" content="">
<meta name="description" content="">
<link rel="stylesheet" type="text/css" href="/wx/Assets/css/reset.css">
<script type="text/javascript" src="/wx/Assets/js/jquery-1.8.3.min.js"></script>
<script type="text/javascript" src="/wx/Assets/js/js_z.js"></script>
<script type="text/javascript" src="/wx/Assets/js/banner.js"></script>
<script type="text/javascript" src="/wx/Assets/js/index.js"></script>
<link rel="stylesheet" type="text/css" href="/wx/Assets/css/thems.css">
<link rel="stylesheet" type="text/css" href="/wx/Assets/css/responsive.css">
<script language="javascript">
$(function(){
    $('#owl-demo').owlCarousel({
        items: 1,
        navigation: true,
        navigationText: ["上一个","下一个"],
        autoPlay: true,
        stopOnHover: true
    }).hover(function(){
        $('.owl-buttons').show();
    }, function(){
        $('.owl-buttons').hide();
    });
});
</script>
<!--[if lte IE 8]>
    <style type="text/css">
        html,body{width:100%; height:100%; overflow:scroll}
        .section-btn{display:none;}
    </style>
    <![endif]-->
<style type="text/css">
body{ overflow:hidden;}
</style>
</head>

<body>
<div class="section-wrap put-section-0">
    <div class="section section-1">
        <div class="sct_a">
            <div id="banner" class="banner"> 
                <div id="owl-demo" class="owl-carousel owl-theme" style="opacity: 1; display: block;"> 
                    <div class="owl-wrapper-outer"><div class="owl-wrapper" style="width: 3312px; left: 0px; display: block; transform: translate3d(0px, 0px, 0px); transition: all 1000ms ease;"><div class="owl-item" style="width: 414px;"><a class="item" target="_blank" href="" style="background-image:url(/wx/Assets/upload/banner_a.jpg)">&nbsp;</a></div><div class="owl-item" style="width: 414px;"><a class="item" target="_blank" href="" style="background-image:url(/wx/Assets/upload/banner_b.jpg)">&nbsp; </a></div><div class="owl-item" style="width: 414px;"><a class="item" target="_blank" href="" style="background-image:url(/wx/Assets/upload/banner_a.jpg)">&nbsp;</a></div><div class="owl-item" style="width: 414px;"><a class="item" target="_blank" href="" style="background-image:url(/wx/Assets/upload/banner_b.jpg)">&nbsp;</a></div></div></div>
                    
                    
                    
                <div class="owl-controls clickable"><div class="owl-pagination"><div class="owl-page active"><span class=""></span></div><div class="owl-page"><span class=""></span></div><div class="owl-page"><span class=""></span></div><div class="owl-page"><span class=""></span></div></div><div class="owl-buttons" style="display: none;"><div class="owl-prev">上一个</div><div class="owl-next">下一个</div></div></div></div>
            </div>
            <div class="i_news">
                <div class="t_news">
                    <b>NEWS：</b>
                    <ul class="news_li" style="top: 0px;">
                        <li>
                            <a href="" target="_blank">
                                <p>乾源通有限公司...</p>
                            </a>
                        </li>
                        <li>
                            <a href="" target="_blank">
                                <p>乾源通有限公司...</p>
                            </a>
                        </li>
                        <li>
                            <a href="" target="_blank">
                                <p>乾源通有限公司...</p>
                            </a>
                        </li>
                    </ul>
                    <ul class="swap" style="top: -57px;">
                        <li>
                            <a href="" target="_blank">
                                <p>乾源通金融有限公司...</p>
                            </a>
                        </li>
                        <li>
                            <a href="" target="_blank">
                                <p>乾源通有限公司...</p>
                            </a>
                        </li>
                        <li>
                            <a href="" target="_blank">
                                <p>乾源通有限公司...</p>
                            </a>
                        </li>
                    </ul>
                    <a href="news.html" class="more">More</a>
                </div>
            </div>
        </div>
    </div>
    <div class="section section-2">
        <div class="i_title">
            <img src="/wx/Assets/images/n1.png" alt="" width="293" height="32">
            <p>了解乾源通</p>
        </div>
        <div class="space_hx">&nbsp;</div>
        <ul class="i_about clearfix">
            <li>
                <a href="/wechat/plist">
                    <div class="tu">
                        <img src="/wx/Assets/images/icon2.png" alt="" class="red">
                        <img src="/wx/Assets/images/icon2_1.png" alt="" class="white">
                    </div>
                    <p>贷款申请</p>
                </a>
            </li>
            <li>
                <a href="/wechat/plist">
                    <div class="tu">
                        <img src="/wx/Assets/images/icon3.png" alt="" class="red">
                        <img src="/wx/Assets/images/icon3_1.png" alt="" class="white">
                    </div>
                    <p>渠道进件</p>
                </a>
            </li>
            <li>
                <a href="/wechat/plist">
                    <div class="tu">
                        <img src="/wx/Assets/images/icon4.png" alt="" class="red">
                        <img src="/wx/Assets/images/icon4_1.png" alt="" class="white">
                    </div>
                    <p>贷款类型</p>
                </a>
            </li>
            <li>
                <a href="/wechat/plist">
                    <div class="tu">
                        <img src="/wx/Assets/images/icon5.png" alt="" class="red">
                        <img src="/wx/Assets/images/icon5_1.png" alt="" class="white">
                    </div>
                    <p>联系我们</p>
                </a>
            </li>
            <li>
                <a href="">
                    <div class="tu">
                        <img src="/wx/Assets/images/icon6.png" alt="" class="red">
                        <img src="/wx/Assets/images/icon6_1.png" alt="" class="white">
                    </div>
                    <p>总裁致辞</p>
                </a>
            </li>
        </ul>
    </div>
    <div class="section section-3">
        <div class="i_title">
            <img src="/wx/Assets/images/n2.png" alt="" width="375" height="32">
            <p>我们的贷款产品？</p>
        </div>
        <div class="space_hx">&nbsp;</div>
        <div class="mainPhoto">
            <span class="goleft nextPage"><a href="javascript:void(0)"><img src="/wx/Assets/images/prev.png" alt=""></a></span>
              <div class="go slidegrid">
                <ul class="slideitems i_service clearfix" style="left: -21.7188px;"><li class="cloned">
                        <div class="tu"><img src="/wx/Assets/images/p5.png" alt=""></div>
                        <div class="wen">
                            <div class="name">社保、公积金贷</div>
                            <p>适用人群——由单位购买了社保或者公积金,放款速度 最快一天,最高额度 50万,贷款期限 1-36个月</p>
                            <a href="service.html">+MORE</a>
                        </div>
                    </li>
                    <li>
                        <div class="tu"><img src="/wx/Assets/images/p1.png" alt=""></div>
                        <div class="wen">
                            <div class="name">汽车抵押贷款</div>
                            <p>适用人群--全款购买的汽车或已结清按揭的车辆,放款速度 最快一天,最高额度 100万,贷款期限 1-36个月</p>
                            <a href="service.html">+MORE</a>
                        </div>
                    </li>
                    <li>
                        <div class="wen">
                            <div class="name">生意贷</div>
                            <p>适用人群——拥有一年以上的营业执照,放款速度 最快一天,最高额度 100万,贷款期限 1-36个月</p>
                            <a href="service.html">+MORE</a>
                        </div>
                        <div class="tu"><img src="/wx/Assets/images/p2.png" alt=""></div>
                    </li>
                    <li>
                        <div class="tu"><img src="/wx/Assets/images/p3.png" alt=""></div>
                        <div class="wen">
                            <div class="name">房屋抵押贷款</div>
                            <p>适用人群——全款购买的房屋,放款速度 最快三天,最高额度  2000万,贷款期限 1-36个月</p>
                            <a href="service.html">+MORE</a>
                        </div>
                    </li>
                    <li>
                        <div class="wen">
                            <div class="name">及时贷</div>
                            <p>适用人群——拥有大陆居民身份证、年满18周岁,放款速度 最快一天,最高额度 3万,贷款期限 1-3个月</p>
                            <a href="service.html">+MORE</a>
                        </div>
                        <div class="tu"><img src="/wx/Assets/images/p4.png" alt=""></div>
                    </li>
                    <li>
                        <div class="tu"><img src="/wx/Assets/images/p5.png" alt=""></div>
                        <div class="wen">
                            <div class="name">社保、公积金贷</div>
                            <p>适用人群——由单位购买了社保或者公积金,放款速度 最快一天,最高额度 50万,贷款期限 1-36个月.</p>
                            <a href="service.html">+MORE</a>
                        </div>
                    </li>
                <li class="cloned">
                        <div class="tu"><img src="/wx/Assets/images/p1.png" alt=""></div>
                        <div class="wen">
                            <div class="name">房屋抵押贷款</div>
                            <p>适用人群——全款购买的房屋,放款速度 最快三天,最高额度  2000万,贷款期限 1-36个月</p>
                            <a href="service.html">+MORE</a>
                        </div>
                    </li></ul>
              </div>
              <span class="goright prevPage"><a href="javascript:void(0)"><img src="/wx/Assets/images/next.png" alt=""></a></span>
        </div>
    </div>
    <div class="section section-4">
        <div class="i_title">
            <img src="/wx/Assets/images/n3.png" alt="" width="360" height="32">
            <p>我们的在线系统！</p>
        </div>
        <div class="space_hx">&nbsp;</div>
        <div class="mainPhoto">
            <span class="goleft nextPage"><a href="javascript:void(0)"><img src="/wx/Assets/images/prev.png" alt=""></a></span>
              <div class="go slidegrid">
                <ul class="slideitems i_new clearfix" style="left: 0px;"><li class="cloned">
                        <a href="/wechat/plist">
                            <div class="tu">
                                <img src="/wx/Assets/upload/pic3.jpg" alt="">
                                <div class="bg_a"><img src="/wx/Assets/images/icon7.png" alt=""></div>
                            </div>
                            <div class="wen">
                                <p>期待你的加入</p>
                                <div class="title active">渠道进件</div>
                                <p class="qian">在线审批，录入</p>
                            </div>
                        </a>
                    </li>
                    <li>
                        <a href="/wechat/plist">
                            <div class="tu">
                                <img src="/wx/Assets/upload/pic1.jpg" alt="">
                                <div class="bg_a"><img src="/wx/Assets/images/icon7.png" alt=""></div>
                            </div>
                            <div class="wen">
                                <p>在线申请</p>
                                <div class="title active">贷款申请</div>
                                <p class="qian">方便快捷，安全可靠</p>
                            </div>
                        </a>
                    </li>
                    <li>
                        <a href="/wechat/plist">
                            <div class="tu">
                                <img src="/wx/Assets/upload/pic2.jpg" alt="">
                                <div class="bg_a"><img src="/wx/Assets/images/icon7.png" alt=""></div>
                            </div>
                            <div class="wen">
                                <p>贷款进度</p>
                                <div class="title active">贷款在线审核</div>
                                <p class="qian">操作简单，省时省心</p>
                            </div>
                        </a>
                    </li>
                    <li>
                        <a href="/wechat/plist">
                            <div class="tu">
                                <img src="/wx/Assets/upload/pic1.jpg" alt="">
                                <div class="bg_a"><img src="/wx/Assets/images/icon7.png" alt=""></div>
                            </div>
                            <div class="wen">
                                <p>在线申请</p>
                                <div class="title active">贷款申请</div>
                                <p class="qian">方便快捷，安全可靠</p>
                            </div>
                        </a>
                    </li>
                <li class="cloned">
                        <a href="/wechat/plist">
                            <div class="tu">
                                <img src="/wx/Assets/upload/pic1.jpg" alt="">
                                <div class="bg_a"><img src="/wx/Assets/images/icon7.png" alt=""></div>
                            </div>
                            <div class="wen">
                                <p>公司新闻</p>
                                <div class="title active">乾源通量化对冲FOF一号契约型基金</div>
                                <p class="qian">量化对冲基金</p>
                            </div>
                        </a>
                    </li></ul>
              </div>
              <span class="goright prevPage"><a href="javascript:void(0)"><img src="/wx/Assets/images/next.png" alt=""></a></span>
          </div>
        <div class="i_more"><a href="/wechat/plist">MORE</a></div>
    </div>
    <div class="section section-5">
        <div class="i_title">
            <img src="/wx/Assets/images/n4.png" alt="" width="387" height="32">
            <p>欢迎您加入乾源通大家庭！</p>
        </div>
        <div class="space_hx">&nbsp;</div>
        <div class="mainPhoto"> 
            <span class="goleft nextPage"><a href="javascript:void(0)"><img src="/wx/Assets/images/prev.png" alt=""></a></span>
              <div class="go slidegrid">
                <ul class="slideitems i_member clearfix" style="left: 0px;"><li class="cloned">
                    <a href="/wechat/plist">
                        <div class="name">行政前台</div>
                        <div class="des">5年以上金融相关工作经验，有3年以上知名投行、投资机构或产业公司...</div>
                        <em>+MOER</em>
                    </a>
                </li>
                    <li>
                    <a href="/wechat/plist">
                        <div class="name">投资经理</div>
                        <div class="des">5年以上金融相关工作经验，有3年以上知名投行、投资机构或产业公司...</div>
                        <em>+MOER</em>
                    </a>
                </li>
                <li>
                    <a href="/wechat/plist">
                        <div class="name">财富业务总监</div>
                        <div class="des">5年以上金融相关工作经验，有3年以上知名投行、投资机构或产业公司...</div>
                        <em>+MOER</em>
                    </a>
                </li>
                <li>
                    <a href="/wechat/plist">
                        <div class="name">金融产品设计师</div>
                        <div class="des">5年以上金融相关工作经验，有3年以上知名投行、投资机构或产业公司...</div>
                        <em>+MOER</em>
                    </a>
                </li>
                <li>
                    <a href="/wechat/plist">
                        <div class="name">行政前台</div>
                        <div class="des">5年以上金融相关工作经验，有3年以上知名投行、投资机构或产业公司...</div>
                        <em>+MOER</em>
                    </a>
                </li>
                <li class="cloned">
                    <a href="/wechat/plist">
                        <div class="name">投资经理</div>
                        <div class="des">5年以上金融相关工作经验，有3年以上知名投行、投资机构或产业公司...</div>
                        <em>+MOER</em>
                    </a>
                </li></ul>
              </div>
              <span class="goright prevPage"><a href="javascript:void(0)"><img src="/wx/Assets/images/next.png" alt=""></a></span>
          </div>
    </div>
</div>
<div class="header">
    <div class="head clearfix">
        <div class="logo"><img src="/wx/Assets/images/logo.png" alt=""></div>
        <div class="nav">
            <div class="nav_ico">&nbsp;</div>
            <ul class="section-btn">
              <li class="on">首页</li>
              <li class="">关于乾源通</li>
              <li class="">我们的业务</li>
              <li class="">新闻中心</li>
              <li class="">加入乾源通</li>
            </ul>
        </div>
        <div class="h_tel"><a href="tel:15623926392"><img src="/wx/Assets/images/tel.png" alt=""></a></div>
    </div>
</div>
<div class="arrow">&nbsp;</div>
<script>
$(function(){
$('.mainPhoto .slidegrid').scrollable({size:1,circular:true,next:'.nextPage',prev:'.prevPage'});
}); 
</script> 

</body></html>